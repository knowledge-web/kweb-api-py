...
if os.path.exists(brain_db_path):
  ...
  # Removing links that don't go anywhere and logging the count
  cursor.execute(
      "SELECT COUNT(*) FROM links WHERE thoughtIdA NOT IN (SELECT Id FROM thoughts) OR thoughtIdB NOT IN (SELECT Id FROM thoughts);"
  )
  invalid_links_count = cursor.fetchone()[0]
  cursor.execute(
      "DELETE FROM links WHERE thoughtIdA NOT IN (SELECT Id FROM thoughts) OR thoughtIdB NOT IN (SELECT Id FROM thoughts);"
  )
  conn.commit()
